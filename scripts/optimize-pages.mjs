...
for(const relative of files){if(mapping.has(relative))continue;const src=path.join(input,relative),dest=path.join(output,relative);fs.mkdirSync(path.dirname(dest),{recursive:true});
  if(/\.(html|css|js|json|xml)$/.test(relative)){let text=fs.readFileSync(src,'utf8');for(const [from,to]of mapping)text=text.replaceAll('/HEOA/'+from,'/HEOA/'+to);fs.writeFileSync(dest,text);}else fs.copyFileSync(src,dest);
}

...
function check(raw,from){
  ...
  const url=new URL(raw,'https://bayesianstvc.github.io/HEOA/'+relative);
  if(!url.pathname.startsWith('/HEOA/')){outsideBase.push({from:relative,url:raw});return;}
  let key=url.pathname.slice('/HEOA/'.length);
...
const report={files:all.length,htmlPages:all.filter(f=>f.endsWith('.html')).length,totalBytes:all.reduce((n,f)=>n+fs.statSync(f).size,0),mediaFiles:media.length,mediaBytes:media.reduce((n,f)=>n+fs.statSync(f).size,0),duplicateMediaBytes:duplicateBytes,checkedLocalUrls:checked.size,missing,outsideBase};
